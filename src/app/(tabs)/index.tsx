...
const OPEN_EVENT_DATE = new Date('2026-05-18T18:00:00');
function getDaysUntilOpen(): number {
  const diff = OPEN_EVENT_DATE.getTime() - new Date().getTime();
  if (diff <= 0) return 0;
...
function OpenEventBanner() {
  ...
  if (days === 0) return (
    <View style={[styles.eventBanner, { backgroundColor: '#000' }]}>
      <View>
        <Text style={styles.eventBannerLabel}>오픈 일자</Text>
        <Text style={styles.eventBannerTitle}>오늘 오후 6시 오픈!</Text>
      </View>
  ...
  return (
    <View style={[styles.eventBanner, { backgroundColor: '#f7f7f7' }]}>
      <View>
        <Text style={[styles.eventBannerLabel, { color: '#b0b8c1' }]}>오픈 일자</Text>
        <Text style={[styles.eventBannerTitle, { color: '#000' }]}>5월 18일 (일) 오후 6시</Text>
      </View>
      <View style={[styles.ddayBadge, { backgroundColor: '#000' }]}>
        <Text style={[styles.ddayText, { color: '#fff' }]}>D-{days}</Text>
      </View>
    </View>
  );
}
...
function NoTeamCard({ name, university, department, studentId, onCreateTeam, onJoinTeam }: {
  name: string; university: string; department: string; studentId: string;
      ...
      {isLeader ? (
        team.applied ? (
          ...
          <View style={styles.actionRow}>
            ...
            <TouchableOpacity style={styles.leaveBtn} onPress={onLeaveTeam}>
              <Text style={styles.leaveBtnText}>팀 나가기</Text>
            </TouchableOpacity>
          </View>
        )
      ) : (
...
export default function HomeScreen() {
  ...
  const isLeader = team
    ? team.members.find(m => m.role === '팀장')?.name === profile?.name
    : false;
      ...
      >
        ...
        {teamLoading || profileLoading ? (
          ...
          <NoTeamCard
            ...
            onJoinTeam={() => setShowCodeInput(true)}
          />
        )}
        ...
        <View style={styles.restaurantsSection}>
          ...
          {restaurants.length === 0 && (
            <Text style={styles.emptyText}>등록된 식당이 없어요</Text>
          )}
        </View>
